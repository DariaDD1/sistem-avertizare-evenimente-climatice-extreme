...
COLOANE_TRASATURI = [
    "temp_aer_min", "temp_aer_medie", "temp_aer_max", "viteza_vant_max",
    "an", "luna", "zi_din_an", "saptamana", "anotimp", "variatie_temp",
    "precip_lag1", "precip_lag2", "precip_lag3", "precip_lag7",
    "precip_sum_3zile", "precip_sum_7zile", "precip_sum_14zile",
    "precip_medie_3zile", "precip_medie_7zile", "precip_medie_14zile",
    "temp_medie_3zile", "temp_medie_7zile", "temp_medie_14zile",
    "temp_std_3zile", "temp_std_7zile", "temp_std_14zile",
    "vant_medie_3zile", "vant_medie_7zile", "vant_medie_14zile",
    "vant_lag1", "vant_lag2", "vant_lag3", "ploaie_ieri",
    "zile_ploioase_consecutive", "zi_calduroasa", "zi_foarte_calduroasa",
    "zi_rece", "zi_vant_puternic", "zi_ploaie_abundenta", "ploaie_persistenta",
]
...
def incarca_fisierele(cai_fisiere):
    return pd.concat([incarca_un_fisier(cale) for cale in cai_fisiere], ignore_index=True)

# ==============================
# PREPROCESARE SI FEATURE ENGINEERING
# ==============================
...
def incarca_si_pregateste_datele(cai_fisiere):
    ...
    coloane_obligatorii = ["data", "temp_aer_min", "temp_aer_medie", "temp_aer_max", "viteza_vant_max", "cant_precipitatii"]
    coloane_lipsa = [coloana for coloana in coloane_obligatorii if coloana not in date.columns]
    if coloane_lipsa:
    ...
    for fereastra in [3, 7, 14]:
        date[f"precip_sum_{fereastra}zile"] = date["cant_precipitatii"].shift(1).rolling(fereastra).sum()
        date[f"precip_medie_{fereastra}zile"] = date["cant_precipitatii"].shift(1).rolling(fereastra).mean()
        date[f"temp_medie_{fereastra}zile"] = date["temp_aer_medie"].shift(1).rolling(fereastra).mean()
        date[f"temp_std_{fereastra}zile"] = date["temp_aer_medie"].shift(1).rolling(fereastra).std()
        date[f"vant_medie_{fereastra}zile"] = date["viteza_vant_max"].shift(1).rolling(fereastra).mean()
    ...
    date["zile_ploioase_consecutive"] = ploaie_anterioara.groupby(grupuri_uscate).cumsum()
    ...
    date["zi_foarte_calduroasa"] = (date["temp_aer_max"] >= prag_temp_max_95).astype(int)
    ...
    date["zi_ploaie_abundenta"] = (date["cant_precipitatii"] >= prag_precip_90).astype(int)
    date["ploaie_persistenta"] = (date["precip_sum_7zile"] >= date["precip_sum_7zile"].quantile(0.90)).astype(int)
    ...
    date.head(5).to_csv(FOLDER_REZULTATE / "primele_5_inregistrari_cu_features.csv", index=False)
    return date
...
if __name__ == "__main__":
    ...
    date_pregatite.to_csv(FOLDER_REZULTATE / "date_meteorologice_procesate.csv", index=False)
