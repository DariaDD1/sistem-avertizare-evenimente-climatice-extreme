...
# Configurare pagina si stilizare
...
if not CALE_MODEL.exists() or not CALE_METADATE.exists() or not CALE_DATE.exists():
    st.error(
        "Lipsesc fisierele modelului antrenat. Ruleaza mai intai: python src/main.py --data-path data/fisier.xlsx"
    )
    st.stop()
...
# Setari praguri si filtrare date

st.sidebar.header("Setari alerte")
prag_implicit_precipitatii = float(date["cant_precipitatii"].quantile(0.90))
prag_precipitatii = st.sidebar.number_input(
    "Prag precipitatii (mm)",
    min_value=0.0,
    max_value=200.0,
    value=prag_implicit_precipitatii,
    step=1.0,
)
prag_probabilitate = st.sidebar.slider(
    "Prag probabilitate ML", min_value=0.0, max_value=1.0, value=0.80, step=0.05
)
prag_scor_risc = st.sidebar.number_input(
    "Prag scor hibrid", min_value=0, max_value=10, value=2, step=1
)

st.sidebar.header("Selectare perioada")
interval_date = st.sidebar.date_input(
    "Perioada analizata",
    value=[date["data"].min().date(), date["data"].max().date()],
    min_value=date["data"].min().date(),
    max_value=date["data"].max().date(),
)
...
date["culoare_precipitatii"] = date["alerta_precipitatii"].map(
    {True: "Peste prag", False: "Normal"}
)
...
# Situație curentă și alertare

st.sidebar.header("Actualizare variabile")
temperatura_minima = st.sidebar.number_input(
    "Temperatura minima", value=float(rand_selectat["temp_aer_min"]), step=0.1
)
temperatura_maxima = st.sidebar.number_input(
    "Temperatura maxima", value=float(rand_selectat["temp_aer_max"]), step=0.1
)
viteza_maxima_vant = st.sidebar.number_input(
    "Viteza maxima vant", value=float(rand_selectat["viteza_vant_max"]), step=0.1
)
precipitatii = st.sidebar.number_input(
    "Precipitatii", value=float(rand_selectat["cant_precipitatii"]), step=0.1
)
...
if (
    precipitatii >= prag_precipitatii
    or probabilitate >= prag_probabilitate
    or situatie_curenta["scor_risc"] >= prag_scor_risc
):
    status = "ALERTA ROSIE"
    culoare_status = "red"
...
if status == "ALERTA ROSIE":
    ...
    if trebuie_trimisa_alerta(status):
        trimite_alerta_email(
            probabilitate,
            precipitatii,
            status,
            situatie_curenta.get("scor_risc"),
            situatie_curenta.get("temp_aer_max"),
            situatie_curenta.get("viteza_vant_max"),
            situatie_curenta.get("zile_ploioase_consecutive"),
        )
...
coloana_kpi1, coloana_kpi2, coloana_kpi3, coloana_kpi4, coloana_kpi5, coloana_kpi6 = (
    st.columns(6)
)
with coloana_kpi1:
    st.metric("Risc ML", f"{probabilitate * 100:.1f}%")
with coloana_kpi2:
    st.metric("Precipitatii", f"{precipitatii:.1f} mm")
with coloana_kpi3:
    st.metric("Vant", f"{viteza_maxima_vant:.1f}")
with coloana_kpi4:
    st.metric("Scor risc", int(situatie_curenta["scor_risc"]))
with coloana_kpi5:
    st.metric("Evenimente extreme", int(date_filtrate["eveniment_extrem"].sum()))
with coloana_kpi6:
    st.metric("Status", status)

tabel_tendinte = (
    date_filtrate.set_index("data")
    .resample("M")
    .agg(
        {
            "cant_precipitatii": "sum",
            "probabilitate_predictie": "mean",
            "scor_risc": "mean",
            "eveniment_extrem": "sum",
        }
    )
    .reset_index()
)
...
with rand1_col1:
    fig = go.Figure(
        go.Indicator(
            mode="gauge",
            value=probabilitate * 100,
            title={"text": "Risc ML (%)", "font": {"size": marime_titlu_grafic}},
            gauge={
                "axis": {"range": [0, 100]},
                "bar": {"color": culoare_status},
                "steps": [
...
with rand1_col2:
    fig = go.Figure(
        go.Indicator(
            mode="gauge",
            value=int(situatie_curenta["scor_risc"]),
            title={"text": "Scor hibrid", "font": {"size": marime_titlu_grafic}},
            gauge={
                "axis": {"range": [0, 8]},
                "bar": {"color": culoare_status},
                "steps": [
...
with rand1_col3:
    fig = px.histogram(
        date_filtrate,
        x="probabilitate_predictie",
        nbins=16,
        color="eveniment_extrem",
        title="Distributia probabilitatilor",
        height=inaltime_grafic,
    )
    fig.add_vline(x=0.30, line_dash="dash", line_color="red")
    fig.update_layout(
        margin=margini_grafic,
        title_font_size=marime_titlu_grafic,
        font=font_grafic,
        showlegend=False,
    )
    st.plotly_chart(fig, use_container_width=True)
...
with rand2_col1:
    evolutie_lunara = (
        date_filtrate.set_index("data")
        .resample("M")
        .agg({"probabilitate_predictie": "mean", "scor_risc": "mean"})
        .reset_index()
    )
    fig = px.line(
        evolutie_lunara,
        x="data",
        y="probabilitate_predictie",
        title="Evolutia lunara a riscului estimat",
        height=inaltime_grafic,
    )
...
with rand2_col2:
    fig = px.bar(
        ...
        x="data",
        y="cant_precipitatii",
        color="culoare_precipitatii",
        color_discrete_map={"Normal": "#4C78A8", "Peste prag": "red"},
        title="Evolutia precipitatiilor",
        height=inaltime_grafic,
    )
...
with rand2_col3:
    fig = px.line(
        ...
        x="data",
        y="scor_risc",
        title="Evolutia scorului hibrid de risc",
        height=inaltime_grafic,
    )
    fig.add_hline(y=prag_scor_risc, line_dash="dash", line_color="red")
    fig.update_layout(
        margin=margini_grafic, title_font_size=marime_titlu_grafic, font=font_grafic
    )
    st.plotly_chart(fig, use_container_width=True)
...
with rand3_col1:
    fig = px.box(
        ...
        y="cant_precipitatii",
        color="eveniment_extrem",
        title="Precipitatii: normal vs extrem",
        height=inaltime_grafic,
    )
    fig.update_layout(
        margin=margini_grafic,
    ...
    st.plotly_chart(fig, use_container_width=True)
with rand3_col2:
    date_temperatura = date_filtrate[
        (date_filtrate["temp_aer_max"] > -50) & (date_filtrate["temp_aer_max"] < 60)
    ].copy()
    fig = px.box(
        date_temperatura,
        x="eveniment_extrem",
        y="temp_aer_max",
        color="eveniment_extrem",
        title="Temperatura maxima: normal vs extrem",
        height=inaltime_grafic,
    )
    fig.update_layout(
        margin=margini_grafic,
...
with rand3_col3:
    fig = px.line(
        tabel_tendinte,
        x="data",
        y=["cant_precipitatii", "eveniment_extrem"],
        title="Tendinta lunara a precipitatiilor si a evenimentelor extreme",
        height=inaltime_grafic,
    )
    fig.update_layout(
        margin=margini_grafic,
